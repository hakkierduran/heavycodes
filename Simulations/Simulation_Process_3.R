...
D=0.25  # variance of random effect
...
for( i in 1:n) {
  ...
  if(X1[i,j] <=5 & X2[i,j]<= 5 ) {
    ...
    y[i,j]=mu_1 + 1*b[i] +  e[i,j]
    
  } else if(X1[i,j] <=5 & X2[i,j]> 5 ) {
    
    y[i,j]=mu_2 +  1*b[i] + e[i,j]  
    
  } else if (X1[i,j] > 5 & X2[i,j]<= 5 ) {
    
    y[i,j]=mu_3 + 1*b[i] +  e[i,j] 
    
  } else if(X1[i,j] > 5 & X2[i,j] > 5 ) {
    
    y[i,j]=mu_4 + 1*b[i] +  e[i,j] 
...
reem_model_heavy<-REEMtree_heavy_mode(y ~X1+X2+X3 , data=simdat ,random=~1, groups=~id,cv=TRUE,family_h=Student(df = 7))
reem_model_heavy
...
reem_model_heavy_t<-REEMtree_heavy_mode(y ~X1+X2+X3 , data=simdat ,random=~1, groups=~id,cv=TRUE,family_h=Student(df = 11))
reem_model_heavy_t
