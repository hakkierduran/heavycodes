...
D=0.5  # variance of random effect
...
reem_model_heavy<-REEMtree_heavy_mode(y ~X1+X2+X3 , data=simdat ,random=~1, groups=~id,cv=TRUE,family_h=Student(df = 11))
reem_model_heavy
...
reem_model_heavy_t<-REEMtree_heavy_mode(y ~X1+X2+X3 , data=simdat ,random=~1, groups=~id,cv=TRUE,family_h=Student(df = 9))
reem_model_heavy_t
