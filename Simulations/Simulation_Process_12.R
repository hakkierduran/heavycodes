...
D = matrix(c(0.5, 0.25, 0.25, 0.5), q, q) # variance of random effect
...
reem_model_heavy<-REEMtree_heavy_mode(y ~X1+X2+X3 , data=simdat ,random=~1, groups=~id,cv=TRUE,family_h=Student(df =4))
reem_model_heavy
...
D=matrix(c(0.5,0.5,0.5,0.25),q,q)   # variance of random effect
I = diag(m)  # variance of errors
...
reem_model_heavy_t<-REEMtree_heavy_mode(y ~X1+X2+X3 , data=simdat ,random=~1, groups=~id,cv=TRUE,family_h=Student(df = 8))
reem_model_heavy_t
